"""FedProx algorithm runner (NVFLARE recipe) for simulation."""
...
def run_fedprox(args) -> None:
    ...
    job_name = args.name if args.name else f"fedprox__{task_short}__alpha{alpha}__mu{prox_mu}"

    print(
        f"Running FedProx ({num_rounds} rounds) task={args.task} alpha={alpha} "
        f"clients={n_clients} mu={prox_mu}"
    )
    ...
    recipe = FedAvgRecipe(
        ...
        aggregator=InTimeAccumulateWeightedAggregator(
            ...
            # Actual FedProx weighting is driven by MetaKey.NUM_STEPS_CURRENT_ROUND sent by clients.
