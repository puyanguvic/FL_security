...
def _prox_term(model: nn.Module, global_model: nn.Module) -> torch.Tensor:
    ...
    while flare.is_running():
        ...
        for epoch in range(args.aggregation_epochs):
            ...
            for data in train_loader:
                ...
                if args.prox_mu > 0:
                    loss = loss + (args.prox_mu / 2.0) * _prox_term(model, global_model)
                loss.backward()
                optimizer.step()
        ...
        meta = {
            ...
            # For FedProx this should reflect the amount of local training data.
