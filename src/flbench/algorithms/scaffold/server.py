...
class ScaffoldServer(BaseServer):
    ...
    def run(self) -> None:
        ...
        job_name = self.args.name if self.args.name else f"scaffold__{task_short}__alpha{alpha}"

        print(
            f"Running SCAFFOLD ({global_rounds} rounds) task={self.args.task} alpha={alpha} "
