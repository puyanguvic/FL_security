"""SCAFFOLD algorithm runner (NVFLARE recipe) for simulation."""
...
def run_scaffold(args) -> None:
    ...
    job_name = args.name if args.name else f"scaffold__{task_short}__alpha{alpha}"

    print(f"Running SCAFFOLD ({num_rounds} rounds) task={args.task} alpha={alpha} clients={n_clients}")
